...
class Matrix{
    ...
    public:
       ...
       Matrix ldlt_decomposition(void){
            ...
            for(int i = 0; i < width; i++){
                for(int j = 0; j <= i; j++){
                    if(i == j){
                        mel tmp = 0;
                        for(int k = 0; k < j; k++)
                            tmp += res.matrix[j][k] * res.matrix[j][k] * res.matrix[k][k];
                        res.matrix[i][i] = matrix[i][i] - tmp;
                    }else{
                        mel tmp = 0;
                        for(int k = 0; k < j; k++)
                            tmp += res.matrix[i][k] * res.matrix[j][k] * res.matrix[k][k];
                        res.matrix[i][j] = 1 / (res.matrix[j][j] * (matrix[i][j] - tmp));
                    }
                }
            }
            return res;
...
int main(){
    Matrix a(cin);
    Matrix res = a.ldlt_decomposition();
    pvec(res.get_matrix());
}
